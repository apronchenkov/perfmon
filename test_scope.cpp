...
BOOST_AUTO_TEST_CASE(test_scope_call)
{
    ...
    BOOST_REQUIRE (counters["scope_call_A"].Ticks() > counters["scope_call_B"].Ticks());
    BOOST_REQUIRE (counters["scope_call_B"].Ticks() > counters["scope_call_C"].Ticks());
    BOOST_REQUIRE (counters["scope_call_C"].Ticks() > 1);

    BOOST_REQUIRE_CLOSE (static_cast<double>(counters["scope_call_A"].Ticks()), static_cast<double>(counters["scope_call_B"].Ticks()), 1.0 );
}
